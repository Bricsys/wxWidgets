...
// Inline the wp_fractional_scale_v1 Wayland protocol interface structures.
// These are normally generated by wayland-scanner from fractional-scale-v1.xml.
extern "C" {
    extern const struct wl_interface wl_surface_interface;
    extern const struct wl_interface wp_fractional_scale_v1_interface;

    static const struct wl_interface* wxWlFracScaleTypes[] = {
        NULL,
        &wp_fractional_scale_v1_interface,
        &wl_surface_interface,
    };
    static const struct wl_message wxWlFracScaleManagerRequests[] = {
        { "destroy",              "",   wxWlFracScaleTypes + 0 },
        { "get_fractional_scale", "no", wxWlFracScaleTypes + 1 },
    };
    const struct wl_interface wp_fractional_scale_manager_v1_interface = {
        "wp_fractional_scale_manager_v1", 1,
        2, wxWlFracScaleManagerRequests,
        0, NULL,
    };
    static const struct wl_message wxWlFracScaleRequests[] = {
        { "destroy", "", wxWlFracScaleTypes + 0 },
    };
    static const struct wl_message wxWlFracScaleEvents[] = {
        { "preferred_scale", "u", wxWlFracScaleTypes + 0 },
    };
    const struct wl_interface wp_fractional_scale_v1_interface = {
        "wp_fractional_scale_v1", 1,
        1, wxWlFracScaleRequests,
        1, wxWlFracScaleEvents,
    };
}

// Minimal inlined client-side API for wp_fractional_scale_v1 (no generated .c needed).
struct wp_fractional_scale_manager_v1;
struct wp_fractional_scale_v1;

struct wp_fractional_scale_v1_listener {
    void (*preferred_scale)(void*, struct wp_fractional_scale_v1*, uint32_t scale);
};

static inline struct wp_fractional_scale_v1*
wxWlGetFractionalScale(struct wp_fractional_scale_manager_v1* mgr, struct wl_surface* surface)
{
    struct wl_proxy* id = wl_proxy_marshal_flags(
        (struct wl_proxy*)mgr,
        1 /* WP_FRACTIONAL_SCALE_MANAGER_V1_GET_FRACTIONAL_SCALE */,
        &wp_fractional_scale_v1_interface,
...
static inline int
wxWlFractionalScaleAddListener(struct wp_fractional_scale_v1* fs,
                                const struct wp_fractional_scale_v1_listener* listener,
                                void* data)
{
    return wl_proxy_add_listener((struct wl_proxy*)fs, (void(**)(void))listener, data);
}
static inline void
wxWlFractionalScaleDestroy(struct wp_fractional_scale_v1* fs)
{
    wl_proxy_marshal_flags((struct wl_proxy*)fs,
        0 /* WP_FRACTIONAL_SCALE_V1_DESTROY */,
        NULL, wl_proxy_get_version((struct wl_proxy*)fs), WL_MARSHAL_FLAG_DESTROY);
}
...
//-----------------------------------------------------------------------------
// Wayland fractional scale support via wp_fractional_scale_v1 protocol
//-----------------------------------------------------------------------------

#ifdef GDK_WINDOWING_WAYLAND

// Per-window state for the fractional scale Wayland object.
struct wxWaylandFractionalScaleState
{
...
struct wp_fractional_scale_manager_v1* gs_wpFractionalScaleManager = NULL;
bool gs_wpFractionalScaleManagerInitialized = false;

// Maps each window to its wp_fractional_scale_v1 state for lifetime management.
WX_DECLARE_HASH_MAP(wxWindowGTK*, wxWaylandFractionalScaleState*,
                    wxPointerHash, wxPointerEqual,
                    wxWaylandFractionalScaleMap);
wxWaylandFractionalScaleMap gs_fsStateMap;

void OnWpFractionalScalePreferredScale(void* data,
                                       struct wp_fractional_scale_v1*,
                                       uint32_t scale)
{
    // scale is in units of 1/120: 120 = 1.0×, 180 = 1.5×, 240 = 2.0×.
    auto* state = static_cast<wxWaylandFractionalScaleState*>(data);
    state->window->m_waylandScaleFactor = scale / 120.0;
}

const struct wp_fractional_scale_v1_listener gs_fsListener = {
    OnWpFractionalScalePreferredScale
};

void OnRegistryGlobal(void*, struct wl_registry* registry,
                      uint32_t id, const char* interface, uint32_t)
{
    if (strcmp(interface, "wp_fractional_scale_manager_v1") == 0)
    {
        gs_wpFractionalScaleManager =
            static_cast<struct wp_fractional_scale_manager_v1*>(
...
const struct wl_registry_listener gs_registryListener = {
    OnRegistryGlobal,
    OnRegistryGlobalRemove
};

// Ensure the wp_fractional_scale_manager_v1 global is bound.
// Safe to call multiple times; does a synchronous roundtrip only once.
void EnsureWpFractionalScaleManager(struct wl_display* wlDisplay)
{
    if (gs_wpFractionalScaleManagerInitialized)
        return;
    gs_wpFractionalScaleManagerInitialized = true;
...
void DestroyWpFractionalScaleForWindow(wxWindowGTK* win)
{
    wxWaylandFractionalScaleMap::iterator it = gs_fsStateMap.find(win);
    if (it == gs_fsStateMap.end())
        return;
    wxWaylandFractionalScaleState* state = it->second;
    gs_fsStateMap.erase(it);
    wxWlFractionalScaleDestroy(state->obj);
    delete state;
}
...
#ifdef GDK_WINDOWING_WAYLAND
    // Only GtkWindow-based widgets own their own wl_surface.  For child
    // widgets, gtk_widget_get_window() returns the nearest ancestor's native
    // GdkWindow, so gdk_wayland_window_get_wl_surface() returns the *same*
...
#ifdef GDK_WINDOWING_WAYLAND
    // Release the wp_fractional_scale_v1 object: the surface is going away.
    DestroyWpFractionalScaleForWindow(this);
    m_waylandScaleFactor = -1.0;
#endif
}
...
#ifdef GDK_WINDOWING_WAYLAND
    // Ensure the fractional scale object is destroyed even if GTKHandleUnrealize
    // was not called (e.g. widget destroyed without being unrealized first).
    DestroyWpFractionalScaleForWindow(this);
#endif
...
{
#ifdef GDK_WINDOWING_WAYLAND
    // On Wayland with GNOME fractional scaling (scale-monitor-framebuffer),
    // the compositor reports an integer GDK scale (e.g. 2) to GTK3 clients
    // even when the user configured a fractional scale (e.g. 1.5×).
    // The actual fractional scale is communicated per-surface via the
    // wp_fractional_scale_v1 Wayland protocol.
    //
    // Only top-level windows have their own wl_surface and receive the
    // preferred_scale event; child windows must defer to their top-level
    // ancestor.  The event also arrives asynchronously (after the surface's
    // first commit, i.e. after the window is mapped), so do one synchronous
    // roundtrip the first time we're asked after the window becomes visible.
    {
        // Walk up to the top-level wxWindowGTK.
        const wxWindowGTK* toplevel = this;
        while (toplevel->m_parent)
            toplevel = static_cast<const wxWindowGTK*>(toplevel->m_parent);

        if (toplevel->m_waylandScaleFactor < 0 &&
            gtk_widget_get_mapped(toplevel->m_widget))
        {
            GdkDisplay* gdkDisplay = gtk_widget_get_display(toplevel->m_widget);
            if (GDK_IS_WAYLAND_DISPLAY(gdkDisplay))
            {
                struct wl_display* wlDisplay =
                    gdk_wayland_display_get_wl_display(gdkDisplay);
        ...
        if (toplevel->m_waylandScaleFactor > 0)
            return toplevel->m_waylandScaleFactor;
    }
#endif // GDK_WINDOWING_WAYLAND
